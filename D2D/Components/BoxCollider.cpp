...
void BoxCollider::Awake()
{
	...
	if (rb && b2Body_IsValid(rb->GetBodyId()))
	{
		...
		float hx = PhysicsUtils::PixelsToMeters(abs(halfScale.x));
		float hy = PhysicsUtils::PixelsToMeters(abs(halfScale.y));
		...
		b2CreatePolygonShape(rb->GetBodyId(), &shapeDef, &box);
	}
}
