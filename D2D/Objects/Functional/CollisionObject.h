...
public:
	...
	float startCalculationDistance = 500.f; //충돌 계산을 시작할 거리. 이보다 멀리 있는 대상과는 충돌 계산을 하지 않음. 단위는 픽셀
protected:
	unordered_map<string, shared_ptr<Collider>> targets; //부딛힐 대상들 (플레이어 등등..) 각자 이름을 가지게 하여 오브젝트를 특정 하거나 중복을 방지, 구분할 수 있도록 함
	unordered_map<string, shared_ptr<Transform>> targetTransforms; //부딛힐 대상들의 Transform 컴포넌트. 충돌 계산 시작 거리를 계산하기 위해 필요. Collider 컴포넌트와 별도로 보관하여 충돌 계산 시작 거리 계산 시 대상의 위치를 매번 GetOwnder()->GetTransform()->GetPosition()으로 찾아서 가져오는 것을 방지하여 성능 향상
